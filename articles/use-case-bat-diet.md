...
`q = 1` (Shannon) and `q = 2` (Simpson). The computation is cumulative:
adding `tree =` returns neutral *and* phylogenetic diversity together in
one tibble, tagged by a `type` column (`type =` keeps just one).
